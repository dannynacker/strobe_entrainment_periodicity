...
desired_frequencies = [7, 8:0.1:12.0, 15]; % Range from 8.0 to 12.0 with a step of 0.1, plus 7 and 15
...
for i = 1:length(desired_frequencies)
    ...
    while iteration < max_iterations
        ...
        % Calculate the effective frequency from the samples
        effective_frequency = calculate_effective_frequency_from_samples(samples, fs);
        
        if abs(effective_frequency - target_Fe) <= tolerance
            best_signal = signal;
            best_Fe = effective_frequency;
            best_F = F;
            break;
        end
        
        % Adjust the frequency based on the effective frequency
        if effective_frequency < target_Fe
            F = F + 0.1;
        else
            F = F - 0.1;
        end
    ...
    save(signal_filename, 'best_signal', 'best_F', 'best_Fe', 'samples', 'ts');
...
function effective_frequency = calculate_effective_frequency_from_samples(samples, fs)
    % Calculate the effective frequency from the samples
    threshold = (max(samples) + min(samples)) / 2;
    flashes = sum(diff(samples > threshold) == 1);
    epoch_length_seconds = length(samples) / fs;
    effective_frequency = flashes / epoch_length_seconds;
end
